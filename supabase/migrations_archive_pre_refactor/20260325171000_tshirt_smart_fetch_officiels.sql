-- Migration: Update Smart Family T-shirt retrieval for Officiels
-- Modifies the get_family_tshirt_info_smart to also return has_registrations = true 
-- when the role is 'officiel', 'juge', 'admin-juge', or 'admin'.
...
BEGIN
    ...
    SELECT 
        ...
        b.taille_tshirt,
        b.t_shirt_recupere,
        ((SELECT COUNT(*) FROM inscriptions i WHERE i.benevole_id = b.id) > 0 OR b.role IN ('juge', 'admin-juge', 'admin', 'officiel'))
    FROM benevoles b
