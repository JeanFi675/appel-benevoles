-- Migration pour sécuriser la RPC d'inscriptions
-- 1. Ajout d'un check explicite auth.uid() IS NULL (Fail Fast)
-- 2. Augmentation du statement_timeout à 30s pour cette fonction
-- 3. Révocation des droits d'exécution pour anon/public
...
-- Ajustement du timeout spécifique pour cette fonction (pour gérer les verrous FOR UPDATE)
ALTER FUNCTION public.manage_inscriptions_transaction(UUID, JSONB)
SET statement_timeout = '30s';

-- Sécurisation des droits d'exécution
REVOKE EXECUTE ON FUNCTION public.manage_inscriptions_transaction(UUID, JSONB) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.manage_inscriptions_transaction(UUID, JSONB) TO authenticated;
