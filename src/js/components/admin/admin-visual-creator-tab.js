...
export function adminVisualCreatorTab() {
    return {
        ...
        addShiftData: {
            lineIndex: -1, titre: '', description: '',
            debut: 8, fin: 12, nb_min: 1, nb_max: 5, referent_id: ''
        },
        showEditShiftModal: false,
        editShiftData: {
            lineIndex: -1, shiftIndex: -1, id: '',
            titre: '', description: '',
            debut: 8, fin: 12, nb_min: 1, nb_max: 5, referent_id: ''
        },
        hoveredShift: null,
        ...
        get postes() { return Alpine.store('admin').postes; },
        get benevoles() { return Alpine.store('admin').benevoles; },
        get periodes() { return Alpine.store('admin').periodes; },
        get dbProgramme() { return Alpine.store('admin').dbProgramme; },
        get dbJours() { return Alpine.store('admin').dbJours; },
        get config() { return Alpine.store('admin').config; },
        get repasList() { return Alpine.store('admin').repasList; },

        get visualDays() { return Alpine.store('admin').visualDays; },
        set visualDays(v) { Alpine.store('admin').visualDays = v; },
        ...
        async deleteVisualDay(day) {
            ...
            try {
                ...
                const dayLabel = dLocal.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
                const dayPrefix = dayLabel.charAt(0).toUpperCase() + dayLabel.slice(1);
                const dayPrefixNoYear = dayPrefix.split(' 202')[0];
                ...
                if (orphanPeriodIds.length > 0) {
                    const { error: orphanError } = await ApiService.delete('periodes', { id: orphanPeriodIds });
                    if (orphanError) {
                        console.error("Erreur lors de la suppression des périodes orphelines du jour :", orphanError);
                    }
                }
