...
- [x] **(Reformulé 2026-06-01 — D8 remplace `supabase db push`)** Appliquer `prod_migration_YYYYMMDD.sql` sur la **prod** via `psql` (et **non** `supabase db push` — inopérant ici : `init.sql` consolidé + migrations atomiques archivées ⇒ `db push` ne rejoue pas les changements structurels et casse sur les vues/fonctions, cf. `audit/notes.md` 2026-05-25/26 D8). **Garde-fous obligatoires :** (1) **backup prod frais** (`supabase db dump`) archivé dans `backups/` *avant* toute écriture ; (2) exécution dans une **transaction** `psql` (`BEGIN; \i prod_migration_YYYYMMDD.sql` puis inspection, `COMMIT` seulement si OK, sinon `ROLLBACK`) ; (3) opération gardée par `PHASE=8` + `--force-prod` + `CONFIRME PROD` explicite du mainteneur. **DoD :** la transaction est `COMMIT`-ée sans erreur, un `pg_dump --schema-only` de la prod correspond désormais au schéma cible, et les comptes des tables cœur sont inchangés. Le `prod_migration_YYYYMMDD.sql` est **jeté** après usage (non archivé dans `migrations/`). — **2026-06-02 : APPLIQUÉ EN PROD** sur `CONFIRME PROD` explicite du mainteneur. Connexion via **Session Pooler IPv4** (`aws-1-eu-west-1.pooler.supabase.com:5432` ; la Direct connection `db.*.supabase.co` est IPv6-only, injoignable depuis le réseau). **Snapshot avant** : `programme` présente / `programmes` absente ; benevoles=140, inscriptions=308, cagnotte_transactions=189. **Backup frais** (lecture seule) : `backups/20260602_PRE-MIGRATION_schema.sql` (58 KB, 14 tables) + `_data.sql` (152 KB, 14 blocs COPY). **Application** : `psql -v ON_ERROR_STOP=1` sur `prod_migration_20260601.sql` (déjà `BEGIN;…COMMIT;`) → **EXIT 0, `COMMIT`, 0 ERROR/FATAL/ROLLBACK**. **Vérifs post-COMMIT** : `programme` ABSENTE / `programmes` présente (dédoublonnée à 20) ; comptes cœur **inchangés** (140/308/189) ; `relforcerowsecurity=true` sur `benevoles` ; 37 policies `public` ; enum `role_type`={benevole,referent,admin}. **Équivalence schéma** : `pg_dump --schema-only` prod-POST vs base locale cible (même pg_dump 17.6, mêmes flags) ⇒ **0 différence DDL** (résiduel = 3 lignes cosmétiques : tokens aléatoires `\restrict`/`\unrestrict` de pg_dump + `COMMENT ON SCHEMA public` par défaut). Artefact `prod_migration_20260601.sql` conservé localement (gitignored) jusqu'à clôture 8.6, non archivé dans `migrations/`. ⚠️ Le mot de passe DB ayant transité en clair pendant l'opération, **à réinitialiser une dernière fois** après la fin de la Phase 8 (aucun composant applicatif ne l'utilise).
- [x] Déployer les Edge Functions : `supabase functions deploy send-planning`, `supabase functions deploy create-benevole`, et **`supabase functions deploy send-relance-orphelin`** (corrigée en 6.4.2 : écriture sur la colonne renommée `orphan_relances.user_id` — son déploiement reste une dette Phase 8, cf. `audit/notes.md` 2026-05-28). **(Ajout 2026-06-01)** Avant déploiement, vérifier que les 2 autres fonctions email (`send-rappel-all`, `send-relance`) ne référencent aucune colonne renommée en Phase 2.6 (`vegetarien`→`is_vegetarien`, `t_shirt_recupere`→`has_recupere_tshirt`, `presence_*`→`is_present_*`, etc.) ; les redéployer si impactées. **DoD :** les fonctions déployées sont visibles et invocables dans le dashboard, et un appel test de `send-relance-orphelin` n'échoue plus sur `orphan_relances`. — **2026-06-02 : FAIT.** **Audit pré-déploiement (read-only)** : grep des 5 fonctions ⇒ aucune référence à un ancien nom de colonne Phase 2.6 ni à la table `programme` (singulier) ; tables interrogées = `benevoles`/`inscriptions`/`orphan_relances` (noms actuels) ; croisement des colonnes `.select()`/`.insert()`/`.upsert()` avec le schéma prod migré = **100 % valides** (`create-benevole` insère `user_id,email,nom,prenom,role` ✓ ; `periodes(nom,ordre)` ✓ ; `send-relance-orphelin` upsert `orphan_relances(user_id, relance_sent_at)` ✓). **Aucune correction de code nécessaire.** **Déploiement** : les 5 fonctions étaient déjà ACTIVE mais à des versions **pré-refacto** (mai/mars 2026) ; **redéployées** depuis la source locale → versions bumpées (`create-benevole` v2→3, `send-planning` v15→16, `send-rappel-all` v3→4, `send-relance` v5→6, `send-relance-orphelin` v2→3), toutes **ACTIVE** au 2026-06-02. Secrets prod déjà tous configurés (SMTP_*, SUPABASE_SERVICE_ROLE_KEY…) — non touchés. **Compatibilité `orphan_relances` prouvée structurellement** (colonne `user_id` présente). **Reste optionnel** : smoke-test live de `send-relance-orphelin` (envoie un email réel + écrit en base) à exécuter par le mainteneur via l'UI admin sur un compte orphelin de test — non déclenché unilatéralement (action sortante).
- [x] **(Ajout 2026-06-01)** Bumper les actions GitHub vers des versions compatibles Node 24 dans `ci.yml` ET `deploy.yml` (`actions/checkout@v4→v5`, `actions/setup-node@v4→v5`, et versions à jour des actions Pages `configure-pages`/`upload-pages-artifact`/`deploy-pages`), avant l'échéance du 16 juin 2026 (Node 20 déprécié, runners forçant Node 24 ; cf. `audit/notes.md` 2026-06-01). À faire sur la branche `refactor/production-hardening` pour que le merge vers `master` embarque le fix dans `deploy.yml`. **DoD :** un run CI sur la branche est vert avec les actions bumpées, et `gh run view` ne montre plus l'annotation de dépréciation Node 20. — **2026-06-02 : FAIT** (commit `6fdfa66`). Versions figées sur l'exemple canonique des docs GitHub Pages (checkout est en réalité à **v6**, pas v5) : `actions/checkout@v4→v6`, `actions/setup-node@v4→v5` + `node-version '20'→'24'` (les deux workflows), `actions/configure-pages@v4→v5`, `actions/upload-pages-artifact@v3→v4` (`deploy.yml`) ; `actions/deploy-pages` déjà en `v4` (inchangé). **Preuve** : run CI `26835678899` (job `quality`) = **success** sur `6fdfa66`, et `gh api .../check-runs` ⇒ `annotations_count: 0` (plus d'annotation de dépréciation Node 20 ; les `DEP0040 punycode`/`DEP0169 url.parse` des logs sont des warnings internes Node émis par des deps npm, sans rapport).
- [x] Merger `refactor/production-hardening` dans `master` via PR (avec revue). **DoD :** la PR est mergée et le CI passe au vert. **(Ajout 2026-06-01)** Le merge doit aussi régulariser les 22 commits `master` local non poussés sur `origin/master` (la branche `refactor/*` partant de ce HEAD local, ils sont embarqués par le merge ; cf. `audit/notes.md` 2026-05-25). **Vérif :** après merge, `git rev-list --count origin/master..master` (côté local synchronisé) = `0`, et le SHA `0321c59` (ou les commits pré-refacto) est présent dans l'historique de `origin/master`. — **2026-06-02 : MERGÉ** (option B validée par le mainteneur — revue = CI verte, mainteneur solo). PR [#1](https://github.com/JeanFi675/appel-benevoles/pull/1) `refactor/production-hardening → master`, **69 commits** (22 pré-refacto + 47 refacto), `origin/master` étant ancêtre direct ⇒ aucun conflit. Stratégie **merge commit** (pas squash, pour préserver l'historique atomique). CI `pull_request` verte (run `26836224354`, EXIT 0) avant merge. Mergé via `gh pr merge --merge` ⇒ merge commit **`acca57e`** (`state: MERGED`, `mergedBy: JeanFi675`, 17:20 UTC). Master local resynchronisé en fast-forward (pas de push direct sur master, hook `pre-push` non sollicité). **Vérifs DoD** : `git rev-list --count origin/master..master` = **0** ; `git merge-base --is-ancestor 239b3db origin/master` (tip des 22 pré-refacto) = **OUI** ; `0321c59` présent dans `origin/master` = **OUI**. Branche `refactor/production-hardening` réalignée en FF sur `acca57e` pour la suite de la Phase 8.
- [x] Vérifier le déploiement frontend (GitHub Actions ou plateforme cible). **DoD :** l'URL de production sert la nouvelle version. — **2026-06-02 : VÉRIFIÉ**. Le merge 8.1 #5 (commit `acca57e` sur `master`) a déclenché `deploy.yml` ⇒ run `26836266849` = **success** (47s), deployment `github-pages` sur sha `acca57e` (17:21 UTC). URL prod **https://jeanfi675.github.io/appel-benevoles/** ⇒ **HTTP 200**, `last-modified: 2026-06-02 17:21:13 GMT` (= horodatage du deploy, build frais). **Preuve « nouvelle version »** : le HTML live référence les chunks du build refactoré — split vendor Phase 5.4 (`vendor-alpine`, `vendor-supabase`, `vendor-qrcode`) + modules séparés Phases 5–6 (`config`, `confirm`, `format-date`, `toast`) — structure absente de l'ancien build monolithique (`main-CqLurZLS.js`, `utils-Ca1yfN8Z.js`). Les assets hashés clés (`main-BPdLDB5Y.js`, `vendor-alpine`, `vendor-supabase`, `config`) résolvent tous en **HTTP 200** (aucune référence périmée/404).
...
- [x] Vérifier les en-têtes de sécurité (CSP, X-Frame-Options, Referrer-Policy). **DoD :** ~~un test sur `securityheaders.com` retourne au minimum un grade `B`~~ **(DoD reformulée 2026-06-02 — option A validée par le mainteneur)** : GitHub Pages n'autorisant aucun en-tête HTTP custom, le grade `securityheaders.com` (qui ne note que les en-têtes HTTP) est **structurellement inatteignable** ; la DoD devient : CSP `<meta>` **active et vérifiée en navigateur sans violation cassant l'appli**, Referrer-Policy posée, et limitation documentée. — **2026-06-02 : VÉRIFIÉ**. CSP + `Referrer-Policy: strict-origin-when-cross-origin` posés en `<meta>` (2 variantes : stricte via `head.html` ; + `cdn.tailwindcss.com` pour `debit`/`scanner-tshirt`). **Validation navigateur** (`npm run preview`, DevTools console) sur les **deux familles** : `scanner-tshirt.html` (variante CDN) → Alpine `init()` s'exécute, CDN Tailwind chargé, page stylée, **zéro violation `script-src`/`style-src`/`font-src`** ; `index.html` (variante stricte) → handler Alpine `requestOtp()` exécuté, **zéro violation hors `connect-src`**. Seul blocage observé : `connect-src` vers `http://127.0.0.1:54321` (Supabase **locale** du build de preview), **faux positif attendu** — en prod les appels REST/Auth/Realtime ciblent `https://pulrflaantftaogvgtnc.supabase.co` (whitelisté). Confirme que le CSP est actif et applique correctement la whitelist. Anti-clickjacking (`X-Frame-Options`/`frame-ancestors`) non couvert (impossible en `<meta>` sur Pages) — documenté dans `docs/deployment.md` + mitigation Cloudflare future. **Amélioration (2026-06-03)** : `connect-src` rendu **dynamique** — injecté au build depuis `VITE_SUPABASE_URL` via `vite.config.js` (`cspConnectSrc`, EJS `<%- %>`, variante `wss`/`ws` dérivée). Plus d'URL en dur ⇒ CSP portable (changement de projet Supabase = changer le secret, zéro édition HTML). Vérifié sur les 2 chemins : build prod → URL prod ; build local → `127.0.0.1`. **Reconfirmation prod post-merge 8.2 (2026-06-03) : FAIT** — PR [#2](https://github.com/JeanFi675/appel-benevoles/pull/2) mergée (commit `27057e8`), `deploy.yml` run `26879470641` = success, site live `HTTP 200` build frais (`last-modified 10:40:29`), CSP servie dans le HTML avec **l'URL prod correctement injectée** (`connect-src 'self' https://pulrflaantftaogvgtnc.supabase.co wss://…`, et non `127.0.0.1` ⇒ injection CI OK). `connect-src` whiteliste l'origine prod (REST/Auth/Realtime) ⇒ données + login non bloqués.
- [x] Mettre à jour `docs/deployment.md` avec les en-têtes de sécurité réels appliqués (CSP, HSTS, X-Frame-Options, Referrer-Policy) et la valeur de chacun. **DoD :** une section "En-têtes de sécurité" est présente et reflète la config prod réelle. — **2026-06-02 : FAIT**. Section « En-têtes de sécurité » ajoutée à `docs/deployment.md` : contrainte GitHub Pages (aucun en-tête HTTP custom), HSTS natif (`max-age=31556952`) + TLS, CSP/Referrer-Policy posés en `<meta>` (2 variantes : stricte via `head.html` ; + `cdn.tailwindcss.com` pour `debit`/`scanner-tshirt`), justification des assouplissements (`'unsafe-eval'` Alpine, `'unsafe-inline'` styles), limitations (anti-clickjacking impossible, grade securityheaders plafonné) et mitigation future (Cloudflare + domaine). Au passage : correction d'une dérive doc (pipeline « Setup Node 20 » → « 24 », aligné sur 8.1 #4).

### 8.3 Monitoring

- [x] Activer les logs Supabase (Database, Auth, Edge Functions) et vérifier qu'ils sont consultables. **DoD :** une requête de test apparaît dans les logs dans la minute. — **2026-06-03 : VÉRIFIÉ**. Les logs Supabase managé (Logflare) sont **toujours actifs** (rien à activer sur le plan) ; la tâche s'est réduite à prouver leur consultabilité via 3 sondes **read-only** sur la prod (aucune écriture, aucun email, aucun compte créé). **Résultats dans le dashboard (Logs & Analytics)** : (1) **API Gateway** → `GET /rest/v1/config` HTTP 200 + `POST /auth/v1/otp` HTTP 422 (client `curl/8.18.0`) ; (2) **Auth** → `POST /otp` à `15:41:33Z`, request_id `019e8e25-a8b4-7573-88cf-8afdf19611b8`, 422 `otp_disabled` (confirme au passage que les **signups OTP sont désactivés en prod**) ; (3) **Postgres/Database** → connexions authentifiées tracées en temps réel ; (4) **Edge Functions** (vue dédiée `send-planning` → onglet *Invocations*, plus fiable que le Logs Explorer) → 5 invocations dont la dernière à `15:48:59Z` (`execution_id cde44bf6…`, status 500, `execution_time_ms 342`). Tous les événements de test sont apparus **sous la minute**. Note : une invocation Edge sans header `Authorization` est rejetée **401 à la passerelle** (n'atteint pas la fonction → absente de `function_edge_logs`) ; il faut un JWT valide (la clé anon suffit) pour que la fonction s'exécute et soit tracée.
- [x] Documenter dans `docs/deployment.md` une section "Monitoring" : où consulter les **logs Supabase** (Database / Auth / Edge Functions) et ce qu'on y trouve. **DoD :** la section "Monitoring" existe et cite les logs Supabase. — **2026-06-03 : FAIT**. Section `## Monitoring` ajoutée à `docs/deployment.md` (après « En-têtes de sécurité ») : tableau des 4 flux (API Gateway / Auth / Postgres-Database / Edge Functions) avec **où** les consulter et **ce qu'on y trouve**, rétention selon le plan (1 j sur Free), piège du 401 passerelle pour les Edge Functions, et 3 sondes `curl` read-only de test (« apparaissent sous la minute »). Cite explicitement les logs Supabase.

### 8.4 Sauvegardes automatiques (plan Free — backup auto-opéré)

> **(Reformulé 2026-06-03)** Le projet reste sur le **plan Supabase Free** : aucun backup automatique managé (PITR/quotidien = Pro+). Le filet de sécurité est donc **auto-opéré** via une GitHub Action planifiée. Contraintes actées : (a) le dump contient des **données personnelles** (≈142 bénévoles) et le repo est **public** ⇒ stockage **chiffré (gpg)** obligatoire, jamais en clair ni commité ; (b) connexion via **Session Pooler IPv4** (`aws-1-eu-west-1.pooler.supabase.com:5432`, la directe est IPv6-only injoignable depuis les runners) ; (c) la chaîne de connexion + la passphrase gpg sont des **secrets GitHub** (le mot de passe DB reste donc actif — sa rotation = mise à jour du secret).

- [x] **(Reformulé 2026-06-03 — plan Free conservé)** Mettre en place une **GitHub Action planifiée** (cron quotidien) qui exécute `supabase db dump` (schéma + données) contre la prod via le Session Pooler et **archive le dump chiffré (gpg) en artifact privé**. **DoD :** le workflow existe, un run manuel (`workflow_dispatch`) produit un dump valide récupérable et déchiffrable, et le cron quotidien est planifié. — **2026-06-04 : FAIT**. `.github/workflows/backup.yml` créé (cron `0 3 * * *` UTC + `workflow_dispatch`), mergé sur `master` via PR [#3](https://github.com/JeanFi675/appel-benevoles/pull/3) (merge commit `99d468d`) — requis car un `schedule` ne se déclenche que depuis la branche par défaut. Le job dump via **`pg_dump 17` (Docker)** contre le **Session Pooler IPv4** (URL passée par env dans le conteneur, jamais en CLI), chiffre en **GPG AES256** et upload **uniquement le `.gpg`** en artifact privé (rétention 30 j, `if-no-files-found: error`, garde-fou taille mini). Secrets posés : `SUPABASE_DB_URL` (pooler), `BACKUP_GPG_PASSPHRASE`. **Run de test** `26945294614` = **success** : dump brut 1 296 828 o, chiffré 334 948 o, artifact `db-backup-20260604_101009` (expire 2026-07-04). **Preuve récupérable + déchiffrable** : artifact téléchargé puis `gpg --decrypt` local → SQL valide (en-tête `pg_dump`, 61 `CREATE TABLE`, tables cœur `benevoles`/`inscriptions`/`cagnotte_transactions`/`postes`/`programmes` présentes, 48 blocs `COPY`, **140 bénévoles + 308 inscriptions** = comptes prod). Clair supprimé après vérif.
- [x] Documenter la procédure de restauration dans `docs/disaster_recovery.md` (en actant : plan Free ⇒ pas de backup managé Supabase, la source de vérité est l'artefact chiffré de la GH Action). **DoD :** le fichier existe et inclut les commandes exactes (dump, déchiffrement gpg, restauration). — **2026-06-04 : FAIT**. `docs/disaster_recovery.md` créé : stratégie (cron quotidien, AES256, rétention 30 j), secrets CI, **récupération** (UI Actions + `gh run download`), **déchiffrement** (`gpg --decrypt` + vérifs intégrité), **restauration** locale (complète + partielle via extraction `awk`) et prod (transaction + `CONFIRME PROD` + dump préalable), vérif que le cron tourne, et §6 réservé au compte-rendu 8.4 #3. Commandes fournies en **bash ET PowerShell** (gpg livré avec Git appelé par chemin complet ; restauration via `Get-Content | docker exec` car `psql` hors PATH et PowerShell sans redirection `<`). `docs/deployment.md` complété : tableau secrets GitHub + 2 lignes `SUPABASE_DB_URL`/`BACKUP_GPG_PASSPHRASE` renvoyant vers `disaster_recovery.md`.
- [x] Tester une restauration partielle sur l'instance Supabase locale à partir d'un backup prod fraîchement téléchargé (issu de la GH Action). **DoD :** un compte-rendu est ajouté à `docs/disaster_recovery.md`. — **2026-06-04 : FAIT**. Restauration partielle de `inscriptions` testée 100 % en local (conteneur `supabase_db_appel-benevoles`, base scratch jetable `dr_test`, prod jamais touchée) depuis le backup chiffré du jour (run `26945294614`). Chaîne complète validée : `gh run download` → `gpg --decrypt` → extraction `awk` (CREATE TABLE sans FK + bloc COPY) → `docker exec psql`. Résultat : `COPY 308` = comptage dump = comptes prod, 0 NULL, UUID/dates valides. Nettoyage : `DROP DATABASE dr_test` + clair supprimé. Compte-rendu daté ajouté au §6 de `docs/disaster_recovery.md`.
...
- [ ] **(Hotfix 2026-06-04 — détecté en recette 8.5 #1)** `besoins.html` : la couleur de remplissage des postes (taux d'inscription : rouge < nb_min, jaune ≥ nb_min, vert ≥ nb_max) et le `sum_inscrits` ne s'affichent que pour `admin`. Cause : `loadInscriptionCounts()` (`src/js/admin-timeline.js`) lit la table `inscriptions` protégée par RLS (compte vide pour non-admin → tout en rouge). Correctif : dériver le compte d'occupation de `public_planning.liste_benevoles` (déjà chargée, publique, complète pour tous les rôles — confirmé en recette : l'info-bulle montre la liste complète) au lieu de la table protégée. **DoD :** un compte `benevole` **et** un compte `referent` voient les postes colorés selon l'occupation réelle (identique à `admin`) ; plus aucun appel à la table `inscriptions` pour ce calcul ; le compteur n'affiche plus `0` à tort.
...
- [ ] **(Ajout 2026-06-02)** Supprimer le Tailwind **runtime CDN** (`<script src="https://cdn.tailwindcss.com">`) des pages `debit.html` et `scanner-tshirt.html` au profit du CSS compilé par Vite (le reste du projet build déjà Tailwind via PostCSS). Découvert en 8.2 #3 (cf. `audit/notes.md` 2026-06-02) : ce CDN runtime est un anti-pattern de prod (perf : compilation Tailwind dans le navigateur ; sécurité : force `https://cdn.tailwindcss.com` + `'unsafe-eval'` dans la CSP de ces deux pages). Sa suppression permet d'**unifier et durcir la CSP** (retirer la variante CDN, aligner sur la CSP stricte de `head.html`). **DoD :** plus aucune référence à `cdn.tailwindcss.com` dans le repo (`grep -r` vide) ; les deux pages s'affichent correctement avec le CSS buildé ; la CSP `<meta>` de ces pages est alignée sur la variante stricte (sans CDN).
